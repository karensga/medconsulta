...
type Especialista = { id: string; name: string };
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        {especialistas.length > 0 && (
          <Select
            value={currentEspecialistaId ?? ALL}
            onValueChange={(val) =>
              router.push(buildUrl({ especialistaId: val && val !== ALL ? val : undefined }))
            }
          >
            <SelectTrigger className="h-7 rounded-full text-sm px-3">
              <SelectValue>
                {currentEspecialistaId && currentEspecialistaId !== ALL
                  ? `Dr. ${especialistas.find((d) => d.id === currentEspecialistaId)?.name ?? ""}`
