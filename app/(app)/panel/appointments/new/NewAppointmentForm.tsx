...
type Especialista = { id: string; name: string; specialty: string; slotDuration: number };
...
export default function NewAppointmentForm({
  ...
  const fetchSlots = async (dId: string, date: string, dur: number) => {
    ...
    try {
      const res = await fetch(`/api/slots?especialistaId=${dId}&date=${date}&duration=${dur}`);
      const data = await res.json();
      setSlots(
  ...
  const handleEspecialistaChange = (id: string) => {
    const doc = especialistas.find((d) => d.id === id);
  ...
  const handleDurationChange = (val: number) => {
    setDuration(val);
    setSelectedSlot(null);
    if (especialistaId && selectedDate) fetchSlots(especialistaId, selectedDate, val);
  };
  ...
  const handleDateChange = async (date: string) => {
    ...
    setSelectedSlot(null);
    setSlots(null);
    if (!especialistaId || !date) return;
    fetchSlots(especialistaId, date, duration);
  };
  ...
  const selectedEspecialista = especialistas.find((d) => d.id === especialistaId);
      ...
      {searched ? (
        <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-5">
          ...
          <div>
            ...
            <Select name="especialistaId" value={especialistaId} onValueChange={(id) => id && handleEspecialistaChange(id)}>
              <SelectTrigger className="w-full h-9 text-sm">
                <SelectValue>
                  {especialistaId
                    ? (() => { const d = especialistas.find((d) => d.id === especialistaId); return d ? `Dr. ${d.name} — ${d.specialty}` : ""; })()
          ...
          {especialistaId && (
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                {selectedEspecialista && duration !== selectedEspecialista.slotDuration && (
                  <button
                    type="button"
                    onClick={() => handleDurationChange(selectedEspecialista.slotDuration)}
                    className="text-xs text-gray-500 hover:text-gray-700 underline"
                  >
                    Restablecer ({selectedEspecialista.slotDuration} min)
                  </button>
                )}
              </div>
            </div>
          ...
          {selectedSlot && selectedEspecialista && (
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="flex items-center gap-2 border border-gray-300 rounded-lg px-3 py-2 text-sm cursor-pointer bg-white has-[:checked]:border-blue-500 has-[:checked]:bg-blue-100">
                    <input type="radio" name="modalidad" value="presencial" defaultChecked required />
                    Presencial
                  </label>
                  <label className="flex items-center gap-2 border border-gray-300 rounded-lg px-3 py-2 text-sm cursor-pointer bg-white has-[:checked]:border-blue-500 has-[:checked]:bg-blue-100">
