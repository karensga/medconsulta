...
export default function SlotPicker({ especialista, availableDates }: Props) {
  ...
  const fetchSlots = async (date: string) => {
    ...
    try {
      const res = await fetch(`/api/slots?especialistaId=${especialista.id}&date=${date}`);
  ...
  const handleSubmit = (formData: FormData) => {
    ...
    setError(null);
    formData.append("especialistaId", especialista.id);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Formulario */}
      {selectedSlot && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-5 animate-in fade-in slide-in-from-bottom-2 duration-200">
          ...
          <form action={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="flex items-center gap-2 border border-gray-300 rounded-lg px-3 py-2 text-sm cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50 transition active:scale-[0.97]"
            >
